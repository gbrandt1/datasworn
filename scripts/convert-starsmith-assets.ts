...
const inputPath =
	'/Users/tvcc/Code/privat/starsmith-compendiums/src/assets/StarsmithAssets.json'
...
for (const asset of data.Assets) {
	const cat =
		...
		asset.Category.toLowerCase().replace(/ /g, '_')
	...
	for (const asset of assets) {
		...
		if (asset.TrackMax) {
			...
			if (asset.TrackCondition) {
				;(assetEntry.controls as Record<string, unknown>)[
