...
import { parseDocument, YAMLMap, Pair, Scalar, isMap, isSeq } from 'yaml';
...
for (const [key, { pair }] of generalOracles) {
...
for (const planetType of PLANET_TYPES) {
  ...
  collection.add(new Pair(new Scalar('name'), new Scalar(snakeToTitle(planetType))));
  ...
  if (parentSource) {
    collection.add(new Pair(new Scalar('_source'), doc.createNode(parentSource)));
  }
