...
const inputPath =
	'/Users/tvcc/Code/privat/ironsmith-compendiums/src/oracles/ironsmith-expanded-oracles.json'
const _outputPath = path.join(
	__dirname,
	'../source_data/ironsmith/oracles.yaml'
)
...
function categorizeOracle(name: string): string {
	...
	if (
		name.startsWith('Fantasy Quest') ||
		name.startsWith('Grim Quest') ||
		name.startsWith('Virtue Challenge') ||
		name.includes('Alternate Fantasy')
	)
		return 'quests'
...
for (const [category, oracles] of Object.entries(oraclesByCategory)) {
	...
	for (const oracle of oracles) {
		...
		const diceMax = parseInt(oracle.d, 10)
...
const existingPath = path.join(
	__dirname,
	'../source_data/ironsmith/assets.yaml'
)
